...
export default function MenuAdminPage({ themeId }: MenuAdminPageProps) {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const res = await apiFetch(withThemeQuery("/api/products", themeId));
      if (res.ok) {
  ...
  const handlePhotoUploadSimulate = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const categoriesList = [
    "Todos",
    ...(isRetail
      ? ["Moda Masculina", "Moda Feminina", "Moda Unissex", "Calçados", "Acessórios"]
      : ["Bolos", "Docinhos", "Tortas", "Bebidas"]
