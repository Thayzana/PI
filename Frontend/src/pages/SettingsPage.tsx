...
import { Settings, RefreshCw, Layers, Shield, Key, Check, AlertCircle, User, Zap } from "lucide-react";
...
export default function SettingsPage({ onResetDatabase, resetting, profile, onProfileUpdate, isAdmin }: SettingsPageProps) {
  ...
  const handleSaveGeminiKey = async (e: FormEvent) => {
    ...
    setKeyMessage(null);
    try {
      const res = await apiFetch("/api/settings/gemini-key", {
        method: "POST",
      ...
      <div className="max-w-2xl bg-white border border-[#eee7de] p-6 rounded-2xl shadow-sm space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-[#faf7f2] border border-[#e5dec9]/60 rounded-xl space-y-3">
            <h4 className="text-xs font-bold text-[#b3543d] uppercase tracking-wider flex items-center gap-1.5">
              <Zap size={14} />
              <span>Modo automático (IA)</span>
            </h4>
            <p className="text-xs text-[#7d6f6b] leading-relaxed">
              Detecta problemas, cria promoções corretivas e simula envio por WhatsApp/Instagram até as APIs serem configuradas.
            </p>
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (res.ok) {
                    setAutoLogs(
                      (data.logs as { step: string; detail: string }[])
                        .map((l) => `${l.step}: ${l.detail}`)
                        .join("\n")
                    );
                  } else {
                    setAutoLogs(data.error || "Falha na automação.");
                  }
                } catch {
                  setAutoLogs("Backend indisponível.");
                } finally {
                  setAutoRunning(false);
                }
              }}
              className="px-4 py-2 bg-[#2e2624] text-white font-bold text-xs rounded-xl hover:opacity-90 cursor-pointer disabled:opacity-60"
            >
              {autoRunning ? "Executando..." : "Executar ciclo agora"}
            </button>
            )}
            {autoLogs && (
              <pre className="text-[10px] bg-white border border-[#eee7de] rounded-lg p-2 whitespace-pre-wrap text-[#5e4f4c]">
                {autoLogs}
              </pre>
            )}
          </div>
