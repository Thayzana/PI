...
- **Runner / Framework**: `vitest`
  - Testes unitarios e de integracao para backend (`Backend/`) e frontend (`Frontend/`).
- **Frontend (React)**:
...
- Fluxo de autenticacao em `Frontend/src/App.tsx`
...
**Modulos-alvo:**
- Endpoints de precificacao em `Backend/routes.ts` / `Backend/...` (cálculo de preco final).
...
Esta versao do plano deve servir como referencia central para a criacao dos testes em `Backend/` e `Frontend/` e para a configuracao de qualquer pipeline de CI que o projeto venha a utilizar.
