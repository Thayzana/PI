...
- **Frontend**
  ...
  - Bundler: Vite (root em `front`).
  - Estilos: Tailwind CSS v4 + tokens de tema em `front/src/index.css`.
...
- **Autenticacao e Seguranca**
  ...
  - Controle de sessao pelo modulo `front/src/lib/auth.ts`.
...
- **Backend**
  - `back/server.ts`: bootstrap do servidor, middlewares, swagger e inicializacao de banco.
  - `back/database/init.ts`: conexao e setup do PostgreSQL.
  - `back/routes.ts`: endpoints REST da aplicacao.
  - `back/gemini.ts`: integracao com Gemini.

- **Frontend**
  - `front/src/App.tsx`: roteamento interno por abas, tema e autenticacao.
  - `front/src/pages/LoginPage.tsx`: tela de login dinamica (fundo alternado, card transluzido e banner de cookies).
  - `front/src/pages/MarketingPage.tsx`: geracao de copys e artes com IA.
  - `front/src/lib/auth.ts`: autenticacao local e persistencia de sessao.
